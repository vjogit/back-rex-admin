...
VALUES ($1, $2, $3, $4)
RETURNING id, ldapid, version, name, surname, email, roles
`
...
const getIdFromLdapid = `-- name: GetIdFromLdapid :one
  SELECT id FROM public.user WHERE ldapid = $1
`

func (q *Queries) GetIdFromLdapid(ctx context.Context, ldapid int32) (int32, error) {
	row := q.db.QueryRow(ctx, getIdFromLdapid, ldapid)
	var id int32
	err := row.Scan(&id)
	return id, err
}
...
SELECT id, ldapid, version, name, surname, email, roles FROM "user"
WHERE id = $1
`
...
SELECT id, ldapid, version, name, surname, email, roles FROM public.user WHERE id = $1
`
...
SELECT id, ldapid, version, name, surname, email, roles FROM public.user ORDER BY id
`
